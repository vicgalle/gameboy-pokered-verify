...
/-- The cost is exactly integer division by 4, matching the two SRL+RR pairs
    in the assembly. -/
theorem L2_cost_is_div4 (n : BitVec 8) :
    (costOf n).toNat = n.toNat / 4 := by native_decide

/-- **Safe-input agreement**: when neither bug applies (cost > 0 and
    currentHP strictly exceeds cost), both implementations agree. -/
theorem L2_agreement_on_safe_inputs :
    ∀ maxHP currentHP : BitVec 8,
    costOf maxHP ≠ 0 → currentHP > costOf maxHP →
    impl maxHP currentHP = spec maxHP currentHP := by native_decide

/-- **Spec ⊆ impl**: whenever spec succeeds, impl also succeeds
    (the buggy impl is strictly more permissive). -/
theorem L2_spec_subset_impl :
    ∀ maxHP currentHP : BitVec 8,
    spec maxHP currentHP ≠ none → impl maxHP currentHP ≠ none := by native_decide

/-- Exactly 4 maxHP values trigger Bug 1 (those with cost = 0: {0, 1, 2, 3}). -/
theorem L2_bug1_maxHP_count :
    (Finset.univ.filter (fun n : BitVec 8 => costOf n = 0)).card = 4 := by native_decide

/-- Exactly 252 (maxHP, currentHP) pairs trigger Bug 2 in isolation:
    one boundary pair per maxHP value with non-zero cost. -/
theorem L2_bug2_trigger_count :
    (Finset.univ.filter (fun p : BitVec 8 × BitVec 8 =>
      costOf p.1 ≠ 0 ∧ p.2 = costOf p.1)).card = 252 := by native_decide

-- ============================================================
-- L3: Minimal fix — verify it matches spec for all inputs
-- ============================================================
...
/-- **Surgical fix**: `implFixed` differs from `impl` only on the exact
    set of buggy inputs — no safe inputs are newly rejected. -/
theorem L3_fix_only_affects_buggy_cases :
    ∀ maxHP currentHP : BitVec 8,
    implFixed maxHP currentHP ≠ impl maxHP currentHP ↔
    costOf maxHP = 0 ∨ currentHP = costOf maxHP := by native_decide

/-- **implFixed ⊆ impl**: whenever the fix succeeds, the original also succeeds.
    The fix only adds rejections, never new acceptances. -/
theorem L3_fixed_subset_of_impl :
    ∀ maxHP currentHP : BitVec 8,
    implFixed maxHP currentHP ≠ none → impl maxHP currentHP ≠ none := by native_decide

/-- The fix eliminates Bug 2: `implFixed` never leaves the user at 0 HP. -/
theorem L3_fixed_never_zero_user_hp :
    ∀ maxHP currentHP newHP subHP : BitVec 8,
    implFixed maxHP currentHP = some (newHP, subHP) → newHP ≠ 0 := by native_decide

/-- The fix eliminates Bug 1: `implFixed` never creates a 0-HP substitute. -/
theorem L3_fixed_never_zero_sub_hp :
    ∀ maxHP currentHP newHP subHP : BitVec 8,
    implFixed maxHP currentHP = some (newHP, subHP) → subHP ≠ 0 := by native_decide
...
/-- **Divergence count**: exactly 1276 input pairs trigger divergence.
    1024 from Bug 1 (4 maxHP values × 256 currentHP values) +
    252 from Bug 2 (one boundary pair per remaining maxHP). -/
theorem L4_divergence_count :
    (Finset.univ.filter (fun p : BitVec 8 × BitVec 8 =>
      impl p.1 p.2 ≠ spec p.1 p.2)).card = 1276 := by native_decide

/-- **HP conservation**: the Substitute operation is lossless in HP terms.
    When spec succeeds, `currentHP = newUserHP + substituteHP`. -/
theorem L4_hp_conservation :
    ∀ maxHP currentHP newHP subHP : BitVec 8,
    spec maxHP currentHP = some (newHP, subHP) →
    currentHP = newHP + subHP := by native_decide

/-- **Cost monotone**: a higher max HP always yields a higher-or-equal substitute cost. -/
theorem L4_cost_monotone :
    ∀ m n : BitVec 8, m ≤ n → costOf m ≤ costOf n := by native_decide

/-- **Spec monotone in current HP**: if a Pokemon with `hp1` current HP can
    create a Substitute, then so can one with any `hp2 ≥ hp1`. -/
theorem L4_spec_monotone_in_hp :
    ∀ maxHP hp1 hp2 : BitVec 8,
    hp1 ≤ hp2 → spec maxHP hp1 ≠ none → spec maxHP hp2 ≠ none := by native_decide

/-- **Substitute HP ceiling**: the substitute can have at most 63 HP
    (quarter of the maximum representable 8-bit HP of 255). -/
theorem L4_substitute_hp_bounded :
    ∀ maxHP : BitVec 8, (costOf maxHP).toNat ≤ 63 := by native_decide

/-- **Fixed impl preserves cost identity**: when `implFixed` succeeds,
    the substitute's HP equals exactly the computed cost. -/
theorem L4_fixed_cost_equals_substitute_hp :
    ∀ maxHP currentHP newHP subHP : BitVec 8,
    implFixed maxHP currentHP = some (newHP, subHP) →
    subHP = costOf maxHP := by native_decide

/-- **Bugs are complementary in failure coverage**: spec rejects strictly more inputs
    than impl's natural carry-only guard; the excess is exactly the 1276 buggy pairs. -/
theorem L4_spec_rejects_more_than_impl :
    (Finset.univ.filter (fun p : BitVec 8 × BitVec 8 =>
      impl p.1 p.2 ≠ none ∧ spec p.1 p.2 = none)).card = 1276 := by native_decide
